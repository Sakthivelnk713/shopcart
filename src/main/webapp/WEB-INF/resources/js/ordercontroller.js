...
cartApp.controller("addToCartCtrl", function ($scope, $http){
    ...
    $scope.removeItemFromCart = function (itemId) {
        $http.put('http://localhost:8090/Shopping/usercart/cart/removeItem/'+itemId).success(function (data) {
    ...
    $scope.addItemToCart = function (itemId) {
        $http.put('http://localhost:8090/Shopping/usercart/cart/addItem/'+itemId).success(function (data) {
    ...
    $scope.refreshCartItems = function () {
        $http.get('http://localhost:8090/Shopping/usercart/cart/refreshCart/'+$scope.cartId).success(function (data) {
           $scope.cart=data;
        });
    };
    ...
    $scope.clearCartItems = function () {
    	$http['delete']('http://localhost:8090/Shopping/usercart/cart/clearCartItems/'+$scope.cartId).success(function (data) {$scope.refreshCartItems()});
    };
});